...
*Setup instructions for a gitian build of VSYNC Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the VSYNC
...
to the VSYNC GitHub release page.
...
- [Building VSYNC Core](#building-vsync-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/vsync-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/vsync-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/vsync-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/vsync-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/vsync-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/vsync-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@vsync.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/vsync-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/vsync-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/vsync-osx-*-build.assert
```
