...
*Setup instructions for a gitian build of CARI Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the CARI
...
to the CARI GitHub release page.
...
- [Building CARI Core](#building-cari-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cari-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cari-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cari-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cari-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cari-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cari-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@cari.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cari-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cari-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cari-osx-*-build.assert
```
