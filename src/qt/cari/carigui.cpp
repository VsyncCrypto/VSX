...
const QString CARIGUI::DEFAULT_WALLET = "~Default";

CARIGUI::CARIGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void CARIGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &CARIGUI::toggleHidden);
...
void CARIGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &CARIGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &CARIGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &CARIGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &CARIGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &CARIGUI::showHide);
...
void CARIGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("CARI Core client") + " " + networkStyle->getTitleAddText();
...
void CARIGUI::handleRestart(QStringList args)
{
...
void CARIGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &CARIGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &CARIGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void CARIGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void CARIGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void CARIGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &CARIGUI::hide);
                e->ignore();
            }
        }
    }
...
void CARIGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void CARIGUI::messageInfo(const QString& text)
{
...
void CARIGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle =  tr("CARI Core"); // default title
...
bool CARIGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("CARI Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void CARIGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void CARIGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CARIGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void CARIGUI::goToSend()
{
    showTop(sendWidget);
}

void CARIGUI::goToAddresses()
{
...
void CARIGUI::goToSettings(){
    showTop(settingsWidget);
}

void CARIGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void CARIGUI::goToReceive()
{
    showTop(receiveWidget);
}

void CARIGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void CARIGUI::showTop(QWidget* view)
{
...
void CARIGUI::changeTheme(bool isLightTheme)
{
...
void CARIGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool CARIGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void CARIGUI::showHide(bool show)
{
...
#ifdef ENABLE_WALLET
bool CARIGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &CARIGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &CARIGUI::message);
    ...
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &CARIGUI::incomingTransaction);

    return true;
}

bool CARIGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void CARIGUI::removeAllWallets()
{
    // Single wallet supported.
}

void CARIGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(CARIGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
