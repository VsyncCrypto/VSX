...
#include "qt/vsync/forms/ui_addresseswidget.h"
#include "qt/vsync/addresslabelrow.h"
#include "qt/vsync/addnewaddressdialog.h"
#include "qt/vsync/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(VSYNCGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination vsyncAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(vsyncAdd, label.toUtf8().constData(),
