...
#include "qt/vsync/dashboardwidget.h"
#include "qt/vsync/forms/ui_dashboardwidget.h"
#include "qt/vsync/sendconfirmdialog.h"
#include "qt/vsync/txrow.h"
...
DashboardWidget::DashboardWidget(VSYNCGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareVsync, "square-chart-vsync");
    setCssProperty(ui->labelSquarezVsx, "square-chart-zvsx");
    setCssProperty(ui->labelVsync, "text-chart-vsync");
    setCssProperty(ui->labelZvsync, "text-chart-zvsx");
    ...
    setCssProperty(ui->labelAmountVsync, "text-stake-vsync-disable");
    setCssProperty(ui->labelAmountZvsync, "text-stake-zvsx-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &VSYNCGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &VSYNCGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &VSYNCGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("VSYNC")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(12,23,11);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isVsync = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZVSYNC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VSYNC, zVSX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            vsync = (pair.first != 0) ? pair.first / 100000000 : 0;
            zvsx = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalVsync += pair.first;
            chartData->totalZvsync += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesVsync.append(vsync);
        chartData->valueszVsx.append(zvsx);

        int max = std::max(vsync, zvsx);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(70,144,62));
    set1->setColor(QColor(129,226,118));
    ...
    set0->append(chartData->valuesVsync);
    set1->append(chartData->valueszVsx);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalVsync > 0 || chartData->totalZvsync > 0) {
        setCssProperty(ui->labelAmountVsync, "text-stake-vsync");
        setCssProperty(ui->labelAmountZvsync, "text-stake-zvsx");
    } else {
        setCssProperty(ui->labelAmountVsync, "text-stake-vsync-disable");
        setCssProperty(ui->labelAmountZvsync, "text-stake-zvsx-disable");
    }
    forceUpdateStyle({ui->labelAmountVsync, ui->labelAmountZvsync});
    ui->labelAmountVsync->setText(GUIUtil::formatBalance(chartData->totalVsync, nDisplayUnit));
    ui->labelAmountZvsync->setText(GUIUtil::formatBalance(chartData->totalZvsync, nDisplayUnit, true));
