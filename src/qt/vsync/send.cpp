...
#include "qt/vsync/forms/ui_send.h"
#include "qt/vsync/addnewcontactdialog.h"
#include "qt/vsync/qtutils.h"
#include "qt/vsync/sendchangeaddressdialog.h"
#include "qt/vsync/optionbutton.h"
#include "qt/vsync/sendconfirmdialog.h"
#include "qt/vsync/myaddressrow.h"
#include "qt/vsync/guitransactionsutils.h"
...
SendWidget::SendWidget(VSYNCGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-vsync");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination vsyncAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(vsyncAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vsyncAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
