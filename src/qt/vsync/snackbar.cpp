...
#include "qt/vsync/forms/ui_snackbar.h"
#include "qt/vsync/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(VSYNCGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &VSYNCGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
