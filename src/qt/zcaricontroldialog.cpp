...
#include "ui_zcaricontroldialog.h"
...
std::set<std::string> ZCariControlDialog::setSelectedMints;
std::set<CMintMeta> ZCariControlDialog::setMints;

bool CZCariControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCariControlDialog::COLUMN_DENOMINATION || column == ZCariControlDialog::COLUMN_VERSION || column == ZCariControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZCariControlDialog::ZCariControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCariControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zCARI Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZCari->setProperty("cssClass", "text-main-purple");
    ui->labelZCari_int->setProperty("cssClass", "text-main-purple");
    ...
    // click on checkbox
    connect(ui->treeWidget, &QTreeWidget::itemChanged, this, &ZCariControlDialog::updateSelection);
    // push select/deselect all button
    connect(ui->pushButtonAll, &QPushButton::clicked, this, &ZCariControlDialog::ButtonAllClicked);
}

ZCariControlDialog::~ZCariControlDialog()
{
    delete ui;
}

void ZCariControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCariControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCariControlWidgetItem* itemDenom(new CZCariControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCariControlWidgetItem *itemMint = new CZCariControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zCARI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCARI seed used to mint this zCARI is not the same as currently hold in the wallet";
            else
...
void ZCariControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCariControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCari_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZCariControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCariControlDialog::GetSelectedMints()
{
...
void ZCariControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
