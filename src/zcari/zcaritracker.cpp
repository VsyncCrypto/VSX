...
CzCARITracker::CzCARITracker(CWallet* parent)
{
    this->wallet = parent;
...
CzCARITracker::~CzCARITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCARITracker::Init()
{
...
bool CzCARITracker::Archive(CMintMeta& meta)
{
...
bool CzCARITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCARITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCARITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCARITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCARITracker::GetSerialHashes()
{
...
CAmount CzCARITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCARITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCARITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCARITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCARITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCARITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCARITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCARITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCARITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCARITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzCARITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCARIWallet* zCARIWallet)
{
    bool iszCARIWalletInitialized = (nullptr != zCARIWallet);
    CMintMeta meta;
    ...
    if (!iszCARIWalletInitialized)
        zCARIWallet = new CzCARIWallet(wallet);
    meta.isSeedCorrect = zCARIWallet->CheckSeed(dMint);
    if (!iszCARIWalletInitialized)
        delete zCARIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCARITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCARITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCARITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCARITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCARITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCARITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzcari from DB\n", __func__, listDeterministicDB.size());
    }
