...
CzCARIWallet::CzCARIWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zcari seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCARISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCARISeed_deprecated()) {
                    LogPrintf("%s: Updated zCARI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcari seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcari wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCARIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCARICount(nCountLastUsed);
    else if (!walletdb.ReadZCARICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCARIWallet::Lock()
{
    seedMaster.SetNull();
}

void CzCARIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCARIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCARI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCARIWallet::LoadMintPoolFromDB()
{
...
void CzCARIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCARIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCARIWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCARIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCARI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCARIWallet::SeedToZCARI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzCARIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCARIWallet::GenerateDeterministicZCARI(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCARIWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCARIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCARIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
