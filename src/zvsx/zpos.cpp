...
bool CLegacyZVsyncStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZVsyncStake from non zc-spend", __func__);
    ...
    *this = CLegacyZVsyncStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zvsx stake origin", __func__);

    // All good
    return true;
}

CLegacyZVsyncStake::CLegacyZVsyncStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZVsyncStake::GetIndexFrom()
{
...
CAmount CLegacyZVsyncStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZVsyncStake::GetUniqueness() const
{
...
bool CLegacyZVsyncStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zVSX stake block: height %d outside range", __func__, nHeight);
