...
CzVSXTracker::CzVSXTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzVSXTracker::~CzVSXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVSXTracker::Init()
{
...
bool CzVSXTracker::Archive(CMintMeta& meta)
{
...
bool CzVSXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVSXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVSXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVSXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzVSXTracker::GetSerialHashes()
{
...
CAmount CzVSXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVSXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVSXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVSXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVSXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVSXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVSXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVSXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVSXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVSXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzVSXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVSXWallet* zVSXWallet)
{
    bool iszVSXWalletInitialized = (nullptr != zVSXWallet);
    CMintMeta meta;
    ...
    if (!iszVSXWalletInitialized)
        zVSXWallet = new CzVSXWallet(wallet);
    meta.isSeedCorrect = zVSXWallet->CheckSeed(dMint);
    if (!iszVSXWalletInitialized)
        delete zVSXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVSXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVSXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVSXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVSXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVSXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVSXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzvsx from DB\n", __func__, listDeterministicDB.size());
    }
