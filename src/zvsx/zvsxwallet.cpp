...
CzVSXWallet::CzVSXWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zvsx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVSYNCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVSYNCSeed_deprecated()) {
                    LogPrintf("%s: Updated zVSX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvsx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvsx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVSXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVSYNCCount(nCountLastUsed);
    else if (!walletdb.ReadZVSYNCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVSXWallet::Lock()
{
    seedMaster.SetNull();
}

void CzVSXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVSXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVSYNC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVSXWallet::LoadMintPoolFromDB()
{
...
void CzVSXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVSXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVSXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zvsxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVSXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVSYNC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvsxTracker which also adds to database
    wallet->zvsxTracker->Add(dMint, true);
...
void CzVSXWallet::SeedToZVSYNC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzVSXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVSXWallet::GenerateDeterministicZVSYNC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVSXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVSXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVSXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
