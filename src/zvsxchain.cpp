...
{
    CDataStream serializedCoinSpend = ZVSYNCModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZVSYNCSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zVSX supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zVSX supply
    ...
    // A one-time event where only the zVSX supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZVSYNCSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zVSX supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zVSX supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
